package 
{
	...
	public class Main extends Sprite 
	{
		...
		private function imageLoaded(event:Event):void
		{
			...
			_badges[currentBadge].icon_url.contentLoaderInfo.removeEventListener(ProgressEvent.PROGRESS, imageLoading);
			_badges[currentBadge].icon_url.contentLoaderInfo.removeEventListener(Event.COMPLETE, imageLoaded);
		...
		private function createSortText():void
		{
			...
			si.addEventListener(MouseEvent.ROLL_OVER, sortOver);
			si.addEventListener(MouseEvent.ROLL_OUT, sortOff);
			si.name = "popular";
			...
			sd.addEventListener(MouseEvent.ROLL_OVER, sortOver);
			sd.addEventListener(MouseEvent.ROLL_OUT, sortOff);
			sd.name = "date";
			...
			sp.addEventListener(MouseEvent.ROLL_OVER, sortOver);
			sp.addEventListener(MouseEvent.ROLL_OUT, sortOff);
			sp.name = "points";
		...
		private function sortOver(event:MouseEvent):void
		{
			event.currentTarget.filters = [new GlowFilter(0xFFFFFF,1,6,6,1,1,false,false)];
			
		}
		private function sortOff(event:MouseEvent):void
		{
			event.currentTarget.filters = [];			
		}
