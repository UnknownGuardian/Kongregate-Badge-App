package 
{
	...
	public class Main extends Sprite 
	{
		...
		{ 
			...
			trace("Number of badges is ", all.length, "You have ", _badges.length, "completed, or ", _badges.length / all.length, "percent complete");
		...
		{
			trace("Attempting to display on screen");
			loadImageConsec();
		}
		...
		private function createTextBox():void
		{
			...
			if(userName == "NA")	//default app catch
			{
				userName = "";
		...
		private function badgeClicked(event:MouseEvent):void
		{
			...
			for (var i:int = 0 ; i < _badges.length; i++)	//find badge, not the border which is display on top of it
			{
		...
		private function displayErrorMessage(type:String = null):void
		{
			...
			stage.addEventListener(KeyboardEvent.KEY_DOWN, pressedAKey);
		...
		private function pressedAKey(event:KeyboardEvent):void
		{
		...
		private function submitUsername(event:MouseEvent = null):void
		{	
			stage.removeEventListener(KeyboardEvent.KEY_DOWN, pressedAKey);
